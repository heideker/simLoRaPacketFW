...
/* ALE */
int sockALE; 
struct sockaddr_in servaddrALE, cliaddrALE; 
...
int main(void)
{
    ...
    /* ALE open socket for incoming traffic from UDP client */
    if ( (sockALE = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) { 
        perror("Erro"); 
        exit(EXIT_FAILURE); 
    } 
    memset(&servaddrALE, 0, sizeof(servaddrALE)); 
    memset(&cliaddrALE, 0, sizeof(cliaddrALE)); 

    servaddrALE.sin_family    = AF_INET; // IPv4 
    servaddrALE.sin_addr.s_addr = INADDR_ANY; 
    servaddrALE.sin_port = htons(1777); 
      
    if ( bind(sockALE, (const struct sockaddr *)&servaddrALE,  
            sizeof(servaddrALE)) < 0 ) 
    { 
        perror("Erro"); 
        exit(EXIT_FAILURE); 
    } 
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        */
        ...
        pthread_mutex_lock(&mx_concent);
        nb_pkt = lgw_receive(NB_PKT_MAX, rxpkt); //AQUI: Trap para função que recebe os pacotes UDP. Deve retornar > 0 (LGW_HAL_ERROR=-1)

	// ALE receive packets and return the number of packets in the buffer...	
	// packet structure:
	struct pacoteUDP {
	...
	char buffer[258];
    	...
    	if (n>0) {
		struct timeval current_unix_time, current_concentrator_time;
		gettimeofday(&current_unix_time, NULL);
                get_concentrator_time(&current_concentrator_time, current_unix_time);
		
		uint32_t time_us = current_concentrator_time.tv_sec * 1000000UL + current_concentrator_time.tv_usec;

        	rxpkt->freq_hz = 870000000;              // uint32_t central frequency of the IF chain
        	rxpkt->if_chain = 1;             //uint8_t   by which IF chain was packet received       
        	rxpkt->status = STAT_CRC_OK;     //uint8_t   status of the received packet       
        	rxpkt->count_us = time_us;             //uint32_t  internal concentrator counter for timestamping, 1 microsecond resolution              
        	rxpkt->rf_chain = 1;             //uint8_t   through which RF chain the packet was received       
        	rxpkt->modulation = MOD_LORA;    //uint8_t   modulation used by the packet        
        	rxpkt->bandwidth = BW_125KHZ;    //uint8_t   modulation bandwidth (LoRa only)        
        	rxpkt->datarate = DR_LORA_SF12;  //uint32_t  RX datarate of the packet (SF for LoRa)        
        	rxpkt->coderate = CR_LORA_4_5;   //uint8_t   error-correcting code of the packet (LoRa only)  
        	rxpkt->rssi = -57;               //float     average packet RSSI in dB     
        	rxpkt->snr = 7;                  //float     average packet SNR, in dB (LoRa only)         
        	rxpkt->snr_min = 7;              //float     minimum packet SNR, in dB (LoRa only)       
        	rxpkt->snr_max = 7;              //float     maximum packet SNR, in dB (LoRa only)       
        	rxpkt->crc = 0;                  //uint16_t  CRC that was received in the payload    
        	rxpkt->size = bufferALE.size;    //uint16_t  payload size in bytes       
        	memcpy(&rxpkt->payload, &bufferALE.payload, bufferALE.size);     //uint8_t buffer containing the payload        
		nb_pkt = 1;
		printf("Payload: size: %d [%s]\n",rxpkt->size, rxpkt->payload); 
	} else {
        	nb_pkt = 0;
	}
